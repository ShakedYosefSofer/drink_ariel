...
router.delete("/:id",auth,async(req,res) => {
  ...
  try{
    ...
    // אם המשתמש אדמין נותן לו למחוק רשומות גם של משתמשים אחרים

    if(req.tokenData.role == "admin" || req.tokenData.role == "superadmin"){

      data = await DrinkModel.deleteOne({_id:id});

    }

    else{

      data = await DrinkModel.deleteOne({_id:id,user_id:req.tokenData._id});
